...
window.Double_Tri = window.classes.Double_Tri =
class Double_Tri extends Shape    // The simplest possible Shape – one triangle.  It has 3 vertices, each
{ constructor()                 // having their own 3D position, normal vector, and texture-space coordinate.
    { super( "positions", "normals", "texture_coords" );                       // Name the values we'll define per each vertex.
                                  // First, specify the vertex positions -- the three point locations of an imaginary triangle.
                                  // Next, supply vectors that point away from the triangle face.  They should match up with the points in 
                                  // the above list.  Normal vectors are needed so the graphics engine can know if the shape is pointed at 
                                  // light or not, and color it accordingly.  lastly, put each point somewhere in texture space too.
      this.positions      = [ Vec.of(-1,0,0), Vec.of(1,0,0), Vec.of(0,1,0) ];
      this.normals        = [ Vec.of(0,0,1), Vec.of(0,0,1), Vec.of(0,0,1) ];
      this.texture_coords = [ Vec.of(0,0),   Vec.of(1,0),   Vec.of(0,1)   ]; 
      this.indices        = [ 0, 1, 2 ];                         // Index into our vertices to connect them into a whole triangle.
                 // A position, normal, and texture coord fully describes one "vertex".  What's the "i"th vertex?  Simply the combined data 
...
window.Rocket = window.classes.Rocket =
class Rocket extends Shape    // A cube inserts six square strips into its arrays.
{ constructor()  
    { super( "positions", "normals", "texture_coords" );   
      ...
      for(var k=0; k<5; k++){
       ...
       for( var i = 0; i < resolution; i++ ){
          var angle = Math.PI*2/resolution;
          var s = (2 * Math.tan(angle/2))/2;
          var transform_matrix = Mat4.identity() 
                                 .times(Mat4.translation(Vec.of(x,y,z ) ))
                                 .times(Mat4.scale([ sx ,sy,sz])) 
                                 

                                                             
                                 .times(Mat4.rotation(angle*i, Vec.of(0,1,0)))                                 
                                 .times(Mat4.translation(Vec.of(0,0,1 ) ))
                                 .times(Mat4.scale([ s ,3,1]))
                                 
                                 ;
          Square.insert_transformed_copy_into( this, [], transform_matrix );
        }
        ...
        for( var i = 0; i < resolution*2; i++ ){
            ...
            var transform_matrix = Mat4.identity() 
                                   ...
                                   .times(Mat4.translation(Vec.of(0, 0, i<resolution? -3:3)))                             
                                   .times(Mat4.rotation(0, Vec.of(0,1,0)))
                                   .times(Mat4.rotation(angle*i, Vec.of(0,0,1) ) )
                                   .times(Mat4.translation(Vec.of(0, -1,0))) 
                                   .times(Mat4.scale([ s ,1,1]))                                   
                                   .times(Mat4.shear(Vec.of(0,0,0),Vec.of(0,0,0),Vec.of(0,i<resolution? -3:0 ,0)  ))                                
                                   ;  

            Double_Tri.insert_transformed_copy_into( this, [], transform_matrix );
        }
